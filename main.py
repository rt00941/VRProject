...
import config
from lib.VirtualRANavigation import *
from lib.Renderer import *
from lib.Scene import *
#from lib.InteractionTechniqueManager import *
...
def start():
    ...
    if sys.platform.startswith('win'):
        os.system('cd hmd-tracker && start \"avango\" cmd /K HMDTracker.exe ' +
                  str(config.HMD_IP_ADDRESS) + ':7770 1000')
        viewing_setup = ViveViewingSetup(scenegraph)

    # create interaction technique
    #interaction_manager = InteractionTechniqueManager()
    #interaction_manager.set_inputs(scenegraph, viewing_setup.camera_node,
